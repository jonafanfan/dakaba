...
**376 tests, ~4 seconds.** No API key needed and no network calls — the OpenAI client is faked, and
...
| [`test_client_loop.py`](tests/test_client_loop.py) | 55 | **the render loop, the marker, the cue ladder, the camera lifecycle and the horizon level's gravity maths, run for real in node** |
...
| [`test_client_overlay.py`](tests/test_client_overlay.py) | 28 | marker drawing, `visibleCrop` maths and the camera-region layout invariants |
