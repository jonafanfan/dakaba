...
The client had no tests at all, and that let a real bug ship in the marker-drawing code the app
used to have. The app no longer draws a standing marker (see CONTRACT.md and README's Known
issues) — positioning guidance is now a single sentence from the model, rendered as plain text —
so the tests that used to run `drawStandMarker` against a stub canvas are gone along with it.
What is left here is `visibleCrop` (still load-bearing: it is what keeps the capture in sync with
what the viewfinder shows) and the layout/region checks below.

`node --check` cannot catch a runtime bug like the marker one: it is valid syntax and only
*running* the code finds it. These tests extract the real function from the shipped page and
execute it, so an out-of-scope identifier fails here instead of on someone's phone.
"""
import json
import re
import shutil
...
def function_source(name):
    """Pull one top-level function out of the page."""
...
def test_visible_crop_matches_the_screen_aspect():
    ...
    When these disagreed, the engine was analysing scenery the user could not see at all.
    """
